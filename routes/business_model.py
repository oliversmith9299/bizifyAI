...
def generate_business_model(user_id: str, db=Depends(get_db)):
    idea_row, problems_row, cust_row, comp_row, mp_row, strat_row = _load_deps(user_id, db)
...
def regenerate_business_model_custom(data: RegenerateCustomInput, db=Depends(get_db)):
    idea_row, problems_row, cust_row, comp_row, mp_row, strat_row = _load_deps(data.user_id, db)
...
def get_business_model(user_id: str, db=Depends(get_db)):
    ...
    return {"user_id": user_id, "business_model": row.data, "chat_history": row.chat_history or []}