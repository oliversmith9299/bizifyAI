...
def generate_mvp_planning(user_id: str, db=Depends(get_db)):
    idea_row, problems_row, cust_row, mp_row, strat_row, bm_row, fl_row = _load_deps(user_id, db)
...
def regenerate_mvp_planning_custom(data: RegenerateCustomInput, db=Depends(get_db)):
    idea_row, problems_row, cust_row, mp_row, strat_row, bm_row, fl_row = _load_deps(data.user_id, db)
...
def get_mvp_planning(user_id: str, db=Depends(get_db)):
    ...
    return {"user_id": user_id, "mvp_planning": row.data, "chat_history": row.chat_history or []}
